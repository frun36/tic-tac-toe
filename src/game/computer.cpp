...
Computer::Computer(Symbol symbol, bool force)
    ...
    Computer::generate_tree(f, 0, 0);
    f.close();

    std::cout << "I've regenerated my database and am ready to annihilate you.\n";
}

bool computer::Computer::position_is_winning(int position, size_t move_count) {
    if (move_count < 4) {
        return false;
    }
    Board board;
    ...
    for (size_t i = move_count % 2; i <= move_count; i += 2) {
        board.set_field(Coordinates(Computer::nth_digit(position, 8 - i)), curr_symbol);
    }
    return board.check_cols() == curr_symbol || board.check_rows() == curr_symbol || board.check_diags() == curr_symbol;
}

int computer::Computer::pow(int base, unsigned int exponent) {
    int result = 1;
    while (exponent > 0) {
        if (exponent % 2 == 0) {
            exponent /= 2;
...
int computer::Computer::nth_digit(int num, int n) {
    return (num / Computer::pow(10, n)) % 10;
}

bool computer::Computer::is_in_number(int num, int digit) {
    for (size_t i = 0; i < 9; i++) {
        if (digit == Computer::nth_digit(num, i)) {
            return true;
        }
    }
    return false;
...
void computer::Computer::generate_tree(std::ofstream& f, int position, size_t move_count) {
    if (move_count >= 9) {
        return;
    }
    for (size_t i = 1; i <= 9; i++) {
        if (Computer::is_in_number(position, i)) {
            continue;
        }
        int new_position = position + i * pow(10, 8 - move_count);
        f << new_position;
        if (Computer::position_is_winning(new_position, move_count)) {
            f << (move_count % 2 == 0 ? "X\n" : "O\n");
            return;
        ...
        Computer::generate_tree(f, new_position, move_count + 1);
    }
}

Coordinates Computer::make_move(const board::Board& board) {
...
void computer::Computer::update_position(const board::Board& board) {
